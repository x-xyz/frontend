...
import { defaultNetwork, getChain, getChainName } from '@x/constants'
...
export default function Menu(props: StackProps) {
  ...
  const { account, chainId = defaultNetwork, deactivate } = useActiveWeb3React()
    ...
    <>
      ...
      <Drawer isOpen={isOpen} onClose={onClose} finalFocusRef={buttonRef}>
        <DrawerContent maxW="100%" height={`calc(100% - ${layout.headerHeight})`} top="initial !important">
          ...
          <DrawerFooter>
            <Text fontSize="sm">©Bazzar 2021</Text>
          </DrawerFooter>
        </DrawerContent>
      </Drawer>
    </>
