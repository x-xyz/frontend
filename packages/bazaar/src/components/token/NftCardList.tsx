...
function NftCardList({
  ...
  const smallColumnCount = useBreakpointValue({ base: 2, md: 3, xl: 5, '2xl': 6, '3xl': 7 }) || 1
  ...
  const rowHeight = useMemo(() => Math.floor(columnWidth + 104), [columnWidth])
      ...
      <ReactWindowScroller isGrid>
        {scrollerProps => (
          <FixedSizeGrid<ItemData>
            ...
            itemData={{ items, columnCount, collections: collections?.data, onMore, isLoading, hasMore }}
            {...scrollerProps}
          >
            {GridItem}
          </FixedSizeGrid>
...
function GridItem({ columnIndex, rowIndex, style, data }: GridChildComponentProps<ItemData>) {
  const { items, columnCount, collections, onMore, isLoading, hasMore } = data

  const index = rowIndex * columnCount + columnIndex

  function render() {
    if (index < items.length) {
      return <NftCardItem token={items[index]} collections={collections} />
    }

    if (!isLoading && !hasMore) return null

    if (index === items.length && onMore) {
      return (
  ...
  return (
    <Box p={2} style={style} transition="left .3s ease-out, top .3s ease-out, width .3s ease-out, height .3s ease-out">
      {render()}
    </Box>
  )
}
...
function NftCardItem({ token, ...props }: NftCardProps) {
  return (
    <Link variant="container" href={`/asset/${token?.contractAddress}/${token?.tokenId}`}>
