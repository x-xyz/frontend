...
export default function SellModal({
  ...
  const onSubmit = handleSubmit(async ({ paymentToken, price, quantity = 1, deadline }) => {
    try {
      if (!isApproved) {
      ...
      if (mode === 'create') {
        ...
        tx = await callContract({
          ...
          args: [contractAddress, tokenID, quantity, token.address, priceInEther, startingTime, deadlineInSeconds],
        })
      } else {
        tx = await callContract({
          contract: marketplaceContract,
          ...
          <chakra.form onSubmit={onSubmit}>
            <ModalBody>
              <Stack spacing={6}>
                ...
                {mode === 'create' && tokenType === TokenType.Erc1155 && (
