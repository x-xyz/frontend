...
import WrapNativeExplanationModal from 'components/WrapNativeExplanationModal'
...
export interface AccountModalContext {
  ...
  wrapNativeExplanationModal: ModalControl
...
const Context = createContext<AccountModalContext>({
  ...
  wrapNativeExplanationModal: noopModalControl,
...
export default function AccountModalProvider({ children }: { children: React.ReactNode }) {
  const { chainId = ChainId.Fantom } = useActiveWeb3React()

  const wrapNativeModal = useDisclosure()

  const wrapNativeExplanationModal = useDisclosure()
  ...
  return (
    <Context.Provider
      value={{
        ...
        wrapNativeExplanationModal: toModalControl(wrapNativeExplanationModal),
    ...
    >
      ...
      <WrapNativeExplanationModal
        chainId={chainId}
        isOpen={wrapNativeExplanationModal.isOpen}
        onClose={wrapNativeExplanationModal.onClose}
      />
