...
export interface AccountMenuProps extends Omit<MenuProps, 'children'> {
...
function WithAccount({ account, children, ...props }: WithAccountProps) {
  const { chainId = ChainId.Fantom, deactivate } = useActiveWeb3React()

  const chain = getChain(chainId)

  const isAdmin = adminWhitelist.includes(account)

  const { data } = useQuery(['account', account], fetchAccountV2)
  ...
  return (
    <>
      <Menu {...props}>
        <MenuButton h={12} p={0} border={children ? 'none' : undefined}>
          {children || (
        ...
        <MenuList>
          ...
          {isFeatureEnabled('vex-page') && isMobile && (
            <Link href="/vex" variant="unstyled">
              <MenuItem disabled>veX Dashboard</MenuItem>
            </Link>
          )}
          ...
          <Link href="/collection/register" variant="unstyled">
            <MenuItem>Register Existing Collection</MenuItem>
          </Link>
          <MenuItem
