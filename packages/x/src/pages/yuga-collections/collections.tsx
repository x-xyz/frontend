...
import * as configs from 'configs/yuga-collections'
...
const wagmi3CollectionAddresses = configs.collections.reduce(
...
function Item({ columnIndex, rowIndex, style, data }: GridChildComponentProps<ItemData>) {
  ...
  function render() {
    if (index < items.length) {
      return <CollectionCard collection={items[index]} hideFloorPrice />
    }

    if (isLoading) {
      return <CollectionCard hideFloorPrice />
    }
  }
