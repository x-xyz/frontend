...
const defaultCollectionWhitelist = builtInCollections
  .map(({ chainId, address }) => ({ chainId, contract: address }))
  .filter(item => validCollections.includes(item.contract))
...
function IpListingItem({ ipListing, onDeleteSuccessful }: IpListingItemProps) {
  ...
  function renderCreatedAt() {
    const style = useDesktopView ? 'long' : 'short'
    return (
      <Text as="span" color="value">
        {ipListing?.createdAt &&
          DateTime.fromISO(ipListing?.createdAt).toLocaleString({ dateStyle: style, timeStyle: style })}
      </Text>
    )
  }
  ...
  return useDesktopView ? (
    ...
    <Grid
      ...
      templateRows={isOpened ? 'repeat(3, auto)' : 'repeat(6, auto)'}
      rowGap={3}
      fontSize="sm"
    >
      <GridItem colStart={2} colSpan={2}>
      ...
      <GridItem>
        <Stack direction="row">
          {renderThumbnail()}
          {!isOpened && (
            <SkeletonText isLoaded={!!ipListing} minW={0}>
      ...
      <GridItem colStart={2}>
        ...
        <Stack direction="row" spacing={6}>
          ...
          <SkeletonText isLoaded={!!ipListing}>{ipListing?.username}</SkeletonText>
        </Stack>
      </GridItem>
      {isOpened && (
        <>
          <GridItem colStart={2} color="value">
            {ipListing?.listingDetail}
          </GridItem>
          <GridItem colStart={2}>
            <Stack direction="row" spacing={5}>
