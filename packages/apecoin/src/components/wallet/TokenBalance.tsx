...
export default function TokenBalance({ chainId = ChainId.Fantom, tokenId, ...props }: TokenBalanceProps) {
  ...
  return (
    <SkeletonText isLoaded={!!balance.value || !balance.isLoading} noOfLines={1}>
      <Text {...props}>
        {`${parseFloat(formatUnits(balance.value, token?.decimals)).toLocaleString(locale, {
