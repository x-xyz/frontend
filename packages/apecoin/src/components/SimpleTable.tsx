...
export default function SimpleTable<T>({
  ...
  function renderColumnName({ key, name, nameToRight }: SimpleField<T>) {
    return (
      <Th
        key={key || `${name}`}
        color="tableHeader"
        textTransform="none"
        whiteSpace="nowrap"
        textAlign={nameToRight ? 'right' : void 0}
      >
        {name}
      </Th>
    )
    ...
    return (
      <Tr key={getKey(item, index)}>
        {visibleFields.map(field => (
          <Td key={field.key || `${field.name}`} whiteSpace="nowrap" textAlign={field.nameToRight ? 'right' : void 0}>
